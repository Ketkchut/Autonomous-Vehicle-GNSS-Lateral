...
class main(object):      
    ...
    def __init__(self):        
        ...
        self.sum_raw_speed = 0
        self.raw_speed_filter = 0

        self.filter_speed_buffer = []
        self.sum_filter_speed = 0
        self.filter_speed_filter = 0
    ...
    def Haversine_filter(self):             #For Latitude/Longitude after filter

        self.now_lat = self.data[3]
        self.now_lon = self.data[4]

        self.prev_lat = self.prev_data[3]
        self.prev_lon = self.prev_data[4]
        ...
        self.distance_km_filter = c*r
        self.distance_m_filter = self.distance_km_filter*1000
    ...
    def Calculate(self):                    #For Calculate Speed
        ...
        if self.now_sec > self.prev_sec:
            ...
            # self.filter_speedms = self.distance_m_filter / (self.now_sec - self.prev_sec)                   #Speed from Filter lat/lon
            # self.filter_speed = round(self.filter_speedms*3.6 ,2)  

    def Talker(self):                       #For Publish Velocity Data

        Velocity = Float64()                     
        Velocity.data = self.raw_speed_filter                   
