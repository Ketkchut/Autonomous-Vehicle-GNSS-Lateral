...
#====== Port conection =============
port1 = "COM40"                                 #port usb that connect rover in your computer for gnss f9p
...
def select_linearEquation(x_north,y_east):
    ...
    elif x_north  <= 1509593.9769328258  and y_east <= 661494.6837220492:       #car on linear1 move on north to sourth
        a,b,c = a3,b3,c3
        print("chose 3 linear a3b3c3")      

    elif  y_east> 661493.5298645048 and y_east <= 661518.5924456262: 
        a,b,c = a4,b4,c4
        print("chose 4 linear a4b4c4")       
    return a,b,c

def cte_current(x_car,y_car,A,B,C):
    global cross_track_error
    print(A,B,C)
    # if y_east< 661494.6837220492:
    #     error1 = abs((A*x_car)+(B*y_car)+C)/math.sqrt(A**2+B**2)
    #     cross_track_error = error1
    #     print("cte on123")
    if  y_east> 661493.5298645048 and y_east <= 661520.5924456262: 
        error2 = abs((A*y_car)+(B*x_car)+C)/math.sqrt(A**2+B**2)
        cross_track_error = error2
        print("cte on4")
...
def cte_positive_negative(y_east,error_cte):
    r = y_east
   #======== Linear North to East =========== move forward on north 150xxx decreasw //cte on 661xxx
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if gngga_data[0] == b"$GNGGA":
            ...
            print(utem_position)
            #(1509628.0198075608, 661489.3068865556) ค่าที่ใกล้ฟุตบาทด้านซ้ายมากทีสุด คือถ้าวิ่งทางตรง จะไม่เกินค่านี้ในแนวนอน
            #(1509582.777557112, 661487.1689644126) ค่ามากสุดทางใต้ ตรงกับฟุตบาทขวาสุดของทางตรงที่สอง หน้าป้าบห้ามเข้า
    #===========================================================================
            # start_time = time.time()                    #บอกเวลา
            # print("time : ", (time.time()-start_time)*1000) #unit = millisecond ไว้บอกเวลาที่ใช้ไปทั้งหมด
            a,b,c = select_linearEquation(x_north,y_east)
           
            error_current = cte_current(x_north,y_east,a,b,c)
            cte_pos_neg = cte_positive_negative(y_east,error_current)    #data can tell cte + -
            print("CTE of Linear equation: %f" %cte_pos_neg)
