...
Kp = 53                                         #best tune in linear waypoint kp ki kd == 50 0.21 18
...
def cte_current(x_car,y_car,A,B,c):
    acceprtable_error = 0
    #1509586.15046655  :left +0.15 |centor : 1509586.00046655 |right -0.15: 1509585.85046655 ///ให้ช่วงนี้ทั้งหมด เป็น error =0
...
def cte_positive_negative(offset_x_north,error_cte):
    #1509588.683282655,2.0034179745327663 max error left
    #1509582.8487296244,-3.780438256067858 max error right 
    global error_positive_negative,cte_previous
    cte_previous = error_positive_negative
    if offset_x_north >= 1509586.42046655 and offset_x_north <= 1509588.4626106732 :      
        error_positive_negative = error_cte     # max 1509588.4626106732,661514.4671315048
    elif offset_x_north<= 1509586.41046655 and offset_x_north >=  1509582.8302920405:    
...
if __name__ == '__main__':
    ...
    #===========================================================================
            offset_x_north = x_north +0.42
            utm_position = offset_x_north,y_east
    ...
    # #cal CTE
            a,b,c =  a4,b4,c4 =  0.006923016, -1 ,1505006.8807523265  
           #661504.5645986241,1509586.0600292445  ค่าทับเส้น จีพีเอสตรงเส้นดำเปะ
            error_current = cte_current(offset_x_north,y_east,a,b,c)
            cte_pos_neg = cte_positive_negative(offset_x_north,error_current)
           
            #start linear 661494.8820608101,1509586.42046655 จุดเริ่มต้นของเส้นตรง เอาจีพีเอสทับเส้นแปะ
    ...
    # #Cal yaw angle that expect from PID controller
            ...
            # test_see = utm_position,cte_pos_neg
    ...
    # #===========input data yaw angle (degree) for control Steering Motor=============
            ...
            data_positioning1 =utm_position
            with open('2_right_CteYaw.csv', 'a', newline='') as f:  
                writer = csv.writer(f,delimiter=",")    
                writer.writerow(Data1) 
            with open('2_right_latlng.csv', 'a', newline='') as f:  
                writer = csv.writer(f,delimiter=",")    
                writer.writerow(gps) 
            with open('2_right_UTM.csv', 'a', newline='') as f:  
                writer = csv.writer(f,delimiter=",")    
