...
{   
  ...
  SpeedControl(velocity_GNSS,Set_speed ,Time_oprate);
...
//-------------------------------------------------------------------//
     ...
     if (adjust > 210) // digitalpoten limit 
      {
        adjust = 210;
      }
//      value =244;
//      runSpeed( state1, state2, state3, value); //call runspeed function open loop       
...
float SpeedControl(float velocity_GNSS,float set,float Time_oprate)
{
    ...
    adjust1 = (error*Kp)+(Ki*sigmaerror)+(Kd*((error-preverror)/dT)) ;
